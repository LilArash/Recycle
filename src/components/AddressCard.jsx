        ...
        <div onClick={() => setShowMap((s) => !s)} className={`flex relative flex-col group ${showMap ? "border border-green-500" : "border"} rounded-xl gap-2 cursor-pointer group`}>
            <div className="flex justify-between items-center ">
                <div className="flex items-center justify-between w-full gap-4 rounded-xl  mr-0 pr-0  h-full relative">
                    <div className={`flex items-center justify-between bg-white h-full rounded-xl w-full hover:w-[90%] transition-all duration-300 py-[1.3rem] ${showMap && "w-[90%]"} ${!showMap && "group-active:border-[3px] group-active:border-green-500 group-active:duration-75 group-active:w-[90%]"} `}>
                        ...
                        <FontAwesomeIcon className={`text-green-500  pl-6 delay-200 ${!showMap && " group-hover:hidden group-hover:delay-1000"} ${showMap && "hidden"}`} icon={faExternalLink} />
                    </div>
